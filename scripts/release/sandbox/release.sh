...
# ║                                                                            ║
# ║  Usage:                                                                    ║
# ║    ./scripts/release/sandbox/release.sh 0.6.0                                              ║
# ║    ./scripts/release/sandbox/release.sh --dry-run 0.6.0                                    ║
# ║    ./scripts/release/sandbox/release.sh --docker 0.6.0                 (include Docker)    ║
# ║    ./scripts/release/sandbox/release.sh --docker --sandbox-only 0.6.0  (Docker sandbox)    ║
# ║                                                                            ║
# ║  Publishes: Sandbox (npm), GitHub Release, Docker (optional)               ║
# ║  Stamps:    packages/sandbox/package.json, scripts/get-kortix.sh                    ║
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      echo "Usage: ./scripts/release/sandbox/release.sh [flags] <version>"
      echo ""
      echo "Flags:"
      ...
      echo "      We pin the CLI version in packages/sandbox/package.json but do not publish it."
      echo ""
      echo "Examples:"
      echo "  ./scripts/release/sandbox/release.sh 0.6.0                                  # Full release"
      echo "  ./scripts/release/sandbox/release.sh --dry-run 0.6.0                        # Validate only"
      echo "  ./scripts/release/sandbox/release.sh --docker 0.6.0                         # All + Docker"
      echo "  ./scripts/release/sandbox/release.sh --docker --sandbox-only 0.6.0          # All + Docker sandbox"
      echo "  ./scripts/release/sandbox/release.sh --docker --skip-daytona 0.6.0          # Docker Hub only"
      exit 0
      ;;
    *)                VERSION="$arg" ;;
  esac
...
if ! $DRY_RUN && ! $BUILD_DOCKER; then
  ...
  echo ""
  echo "  Use one of:"
  echo "    ./scripts/release/sandbox/release.sh --docker $VERSION"
  echo "    ./scripts/release/sandbox/release.sh --dry-run $VERSION"
  exit 1
fi
