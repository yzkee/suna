...
export function ApifyApprovalInline({ approval, threadId, onApproved }: ApifyApprovalInlineProps) {
  ...
  // Use updated approval from query if available, otherwise use prop
  ...
  const maxCostCredits = currentApproval.estimated_cost_credits || (maxCostUsd * 100 * 1.2); // 20% markup
  ...
  return (
    <div className="my-2 p-3 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900/50">
      <div className="space-y-2.5">
        ...
        {/* Cost Display - Clean, Credit-First */}
          ...
          {currentApproval.estimated_cost_usd && currentApproval.estimated_cost_usd !== maxCostUsd && (
            <div className="pt-1.5 mt-1.5 border-t border-zinc-200 dark:border-zinc-700">
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="text-base font-semibold text-zinc-700 dark:text-zinc-300 tabular-nums">
                  {formatCredits(currentApproval.estimated_cost_credits)}
                </span>
                <span className="text-xs text-zinc-500 dark:text-zinc-500">
                  credits ≈ ${formatUSD(currentApproval.estimated_cost_usd)}
                </span>
              </div>
            </div>
          )}
