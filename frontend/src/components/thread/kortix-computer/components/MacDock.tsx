...
interface DockProps {
  children: ReactNode | ((mouseX: MotionValue) => ReactNode);
  className?: string;
}
...
export const DockCard = memo(function DockCard({
  ...
  const borderRadiusSync = useTransform(
    distance,
    [-MAGNIFICATION_DISTANCE, 0, MAGNIFICATION_DISTANCE],
    [16, 22, 16]
  );

  const borderRadius = useSpring(borderRadiusSync, SPRING_CONFIG);
  ...
  return (
    <div className="relative flex flex-col items-center group">
      <motion.button
        ref={ref}
        type="button"
        style={{ width, height, y, borderRadius }}
        onClick={() => {
          onClick?.();
        }}
        className={cn(
          "flex items-center justify-center relative cursor-pointer p-2",
          "duration-75",
          isExternalApp
      ...
      {isFailed && (
        <div className="absolute -top-1 -right-1 w-5 h-5 bg-red-400 rounded-full flex items-center justify-center border-2 border-white z-10">
      ...
      <motion.div 
        className={cn(
          "absolute -top-14 left-1/2 -translate-x-1/2 px-3 py-1.5 text-white text-xs font-medium rounded-lg whitespace-nowrap pointer-events-none z-50",
          "bg-black/70 backdrop-blur-xl border border-white/10",
          "opacity-0 group-hover:opacity-100 transition-opacity duration-150"
        )}
        style={{ 
          y: useSpring(useTransform(distance, [-MAGNIFICATION_DISTANCE, 0, MAGNIFICATION_DISTANCE], [4, 0, 4]), SPRING_CONFIG),
  ...
  return (
    <div className="relative flex flex-col items-center group">
      <motion.button
        ...
        className={cn(
          "flex items-center justify-center relative cursor-pointer p-2",
          "duration-75 shadow-lg border border-white/20",
          bgClass
        )}
        whileTap={{ scale: 0.92 }}
      >
    ...
    >
      <div className="relative flex items-center justify-center py-3 px-4 max-w-full">
        ...
        <Dock>
          {(mouseX: MotionValue) => (
            <>
              {/* System Apps - Only in maximized mode */}
              {isMaximized && (
                <>
                  {showFilesTab && (
                    <SystemDockCard
