      ...
      <CardContent className="space-y-3">
        ...
        <div className="text-xs text-zinc-500 dark:text-zinc-400 space-y-1">
          {currentApproval.created_at && (
            <p>Created: {new Date(currentApproval.created_at).toLocaleString()}</p>
          )}
          {currentApproval.approved_at && (
            <p>Approved: {new Date(currentApproval.approved_at).toLocaleString()}</p>
          )}
          {currentApproval.expires_at && (
            <p>Expires: {new Date(currentApproval.expires_at).toLocaleString()}</p>
          )}
        </div>
      </CardContent>
    </Card>
