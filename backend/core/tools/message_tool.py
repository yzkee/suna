            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "follow_up_answers": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "🚨 MANDATORY for clarification questions - users should click answers, not type them. Array of suggested follow-up answer strings (2-4 options). MUST be valid JSON array format: [\"answer1\", \"answer2\"]. CRITICAL GUIDELINES: 1) REQUIRED for all clarification questions - reduce friction by providing clickable options, 2) Keep answers CONCISE (1-2 lines max) - users should scan quickly, 3) Make answers SPECIFIC - reference actual options, files, or choices (e.g., 'Use Python with FastAPI for the backend' not 'Option A'), 4) Include brief reasoning when helpful (e.g., 'Yes, deploy to production - tests passing' not just 'Yes'), 5) For yes/no questions, include context (e.g., 'Yes, proceed with dark theme' not just 'Yes'), 6) Avoid generic responses like 'Yes', 'No', 'Option A' - make them descriptive and contextual. GOOD EXAMPLES: 'Yes, create React component with TypeScript', 'Skip tests for now and deploy', 'Use existing API endpoint instead'. BAD EXAMPLES: 'Yes', 'No', 'Option 1', 'Proceed'. Maximum 4 suggestions, each should be self-explanatory when read standalone."
                    }
                },
                "required": ["text"]
            }
