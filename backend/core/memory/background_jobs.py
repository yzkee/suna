...
async def extract_memories(thread_id: str, account_id: str, message_ids: List[str]):
    ...
    from core.services.stream_worker import dispatch_memory_extraction
...
async def consolidate_memories(account_id: str):
    ...
    from core.services.stream_worker import dispatch_memory_consolidation
...
extract_memories_from_conversation = _DispatchWrapper(
    lambda thread_id, account_id, message_ids: 
        __import__('core.services.stream_worker', fromlist=['dispatch_memory_extraction']).dispatch_memory_extraction(thread_id, account_id, message_ids)
)

embed_and_store_memories = _DispatchWrapper(
    lambda account_id, thread_id, extracted_memories:
        __import__('core.services.stream_worker', fromlist=['dispatch_memory_embedding']).dispatch_memory_embedding(account_id, thread_id, extracted_memories)
)
