        ...
        try:
            ...
            if should_save_message:
                ...
                if last_assistant_message_object and last_assistant_message_object.get('message_id'):
                    ...
                    # Get DB client - prefer thread_manager if available, otherwise create new connection
                    if self.thread_manager and hasattr(self.thread_manager, 'db'):
                    ...
                    try:
                        ...
                        # Only use lock if thread_manager has the method
                        if self.thread_manager and hasattr(self.thread_manager, 'get_thread_lock'):
                            thread_lock = await self.thread_manager.get_thread_lock(thread_id)
                            ...
                            # No lock available, proceed without it
                            result = await client.table('messages').update({
                                'content': message_data,
                                'metadata': assistant_metadata,
                                'updated_at': datetime.now(timezone.utc).isoformat()
