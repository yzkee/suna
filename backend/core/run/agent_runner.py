...
class AgentRunner:
    ...
    async def run(self, cancellation_event: Optional[asyncio.Event] = None) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        if (hasattr(self.thread_manager, 'mcp_loader') and 
            ...
            logger.info("⚡ [MCP JIT] Using JIT MCP system, completely disabling legacy MCP")
            self._clean_legacy_mcp_tools()
            mcp_wrapper_instance = None
            await setup_tools_task
        else:
            logger.info("⚠️  [MCP] No JIT MCP loader or configs, using legacy system")
        ...
        if mcp_wrapper_instance:
            logger.info(f"⏱️ [TIMING] Tool setup: {tools_elapsed:.1f}ms | Legacy MCP")
        else:
            logger.info(f"⏱️ [TIMING] Tool setup: {tools_elapsed:.1f}ms | JIT MCP")
