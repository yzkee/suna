...
BEGIN
    ...
    FOR v_account IN 
        ...
        AND (
            (ca.provider = 'stripe' AND ca.stripe_subscription_id IS NOT NULL AND ca.stripe_subscription_status = 'active')
            OR
            (ca.provider = 'revenuecat' AND ca.revenuecat_subscription_id IS NOT NULL)
        )
    ...
    LOOP
        BEGIN
            ...
            v_subscription_id := COALESCE(v_account.stripe_subscription_id, v_account.revenuecat_subscription_id, 'unknown');
            ...
            RAISE NOTICE '[YEARLY REFILL] Processing account % (provider: %) - tier: %, credits: %, period: % to % (month %/12)', 
                v_account.account_id, v_account.provider, v_account.tier, v_monthly_credits, v_account.next_credit_grant, v_next_grant, v_months_since_start + 1;
