 ...
 * These are API keys and credentials that OpenCode needs inside the container.
 */
...
export class LocalDockerProvider implements SandboxProvider {
  ...
  async create(opts: CreateSandboxOpts): Promise<ProvisionResult> {
    ...
    // Read API keys and credentials from sandbox/.env
    ...
    const env = [
      ...
      `PROJECT_ID=local`,
      `ENV_MODE=local`,
      // API keys and credentials from sandbox/.env
      ...sandboxEnvVars,
      // Pass through any extra env vars from the caller (highest priority)
  ...
  /**
   * List all kortix sandbox containers (running + stopped).
   * Uses the `kortix.sandbox=true` label for discovery.
   * Returns container info including current port mappings.
   */
  async list(): Promise<DockerSandboxInfo[]> {
    const containers = await this.docker.listContainers({
      all: true, // include stopped
      filters: { label: ['kortix.sandbox=true'] },
    });

    const results: DockerSandboxInfo[] = [];

    for (const c of containers) {
      // Port mappings from listContainers are in a different format — inspect for accuracy
      const mappedPorts: Record<string, string> = {};
      let baseUrl = '';
      ...
      if (c.State === 'running') {
        try {
          ...
          baseUrl = mappedPorts['8000']
            ? `http://localhost:${mappedPorts['8000']}`
            : '';
        } catch {
          // Container may have died between list and inspect
        }
      }

      const status: SandboxStatus =
        c.State === 'running' ? 'running' :
        c.State === 'exited' || c.State === 'created' ? 'stopped' :
        'unknown';

      // Container name from Docker (strip leading /)
  ...
  /**
   * Inspect a single container and return its current info (including refreshed ports).
   */
  async inspect(externalId: string): Promise<DockerSandboxInfo> {
    const container = this.docker.getContainer(externalId);
    const info = await container.inspect();
    ...
    const baseUrl = mappedPorts['8000']
      ? `http://localhost:${mappedPorts['8000']}`
      : '';
...
/** Container info returned by list() and inspect(). */
export interface DockerSandboxInfo {
  containerId: string;
  name: string;
