...
Uses Drizzle ORM with a dedicated `kortix_channels` Postgres schema (same pattern as `kortix_cron`).

```typescript
// db/schema.ts

import { pgSchema, uuid, text, boolean, timestamp, jsonb, index } from 'drizzle-orm/pg-core';

export const kortixChannels = pgSchema('kortix_channels');

// ─── Channel type enum ───────────────────────────────────────────────────────
export const channelTypeEnum = kortixChannels.enum('channel_type', [
  'telegram',
  'slack',
  'discord',
  'whatsapp',
...
export const sessionStrategyEnum = kortixChannels.enum('session_strategy', [
  'single',
  'per-thread',
  'per-user',
  'per-message',
...
export const channelConfigs = kortixChannels.table(
  'channel_configs',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    accountId: uuid('account_id').notNull(),         // auth.users FK
    sandboxId: text('sandbox_id').notNull(),          // Daytona workspace ID
    channelType: channelTypeEnum('channel_type').notNull(),
    name: text('name').notNull(),                     // User-given name
    enabled: boolean('enabled').default(true).notNull(),

    // Platform credentials (bot token, webhook secret, etc.)
    // Encrypted at rest by Supabase column encryption or application-level
    credentials: jsonb('credentials').notNull().$type<Record<string, string>>(),

    // Sandbox connection details (copied from sandbox registry)
    sandboxBaseUrl: text('sandbox_base_url').notNull(),
    sandboxAuthToken: text('sandbox_auth_token'),

    // Platform-specific config (allowed channels, user mappings, group settings, etc.)
    platformConfig: jsonb('platform_config').default({}).$type<Record<string, unknown>>(),

    // Session routing
    sessionStrategy: sessionStrategyEnum('session_strategy').default('per-thread').notNull(),
    systemPrompt: text('system_prompt'),              // Optional prefix for all messages
    agentName: text('agent_name'),                    // Override agent (default: general)
...
export const channelSessions = kortixChannels.table(
  'channel_sessions',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const channelMessages = kortixChannels.table(
  'channel_messages',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    channelConfigId: uuid('channel_config_id')
    ...
    platformUser: jsonb('platform_user').$type<{ id: string; name: string }>(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [
...
export const channelIdentityMap = kortixChannels.table(
  'channel_identity_map',
  {
    ...
    kortixUserId: uuid('kortix_user_id'),              // Nullable — not all platform users have accounts
    allowed: boolean('allowed').default(true).notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [
    index('idx_identity_config_user').on(table.channelConfigId, table.platformUserId),
  ],
);
```
...
The `channel_configs` table stores `sandbox_id`, `sandbox_base_url`, and `sandbox_auth_token` directly. This duplicates data from the sandbox registry but avoids cross-service queries at message time.
...
│   │   ├── schema.ts                     # Tables defined in section 7
...
6. Implement Drizzle schema + migrations
...
4. **Shared sandbox registry:** Both `kortix-cron` and `kortix-channels` need sandbox connection details (`base_url`, `auth_token`). Should we:
   - (a) Duplicate in each service's DB (current plan — `sandboxBaseUrl` in `channel_configs`)
   - (b) Create a shared `kortix_core.sandboxes` table
   - (c) Call `kortix-cloud` API at runtime to resolve
