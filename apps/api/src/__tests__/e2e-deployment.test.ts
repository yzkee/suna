...
// Resolve repo root dynamically from this file's location: __tests__ -> src -> api -> apps -> computer/
const REPO_ROOT = resolve(__dirname, '..', '..', '..', '..');
const SANDBOX_DIR = join(REPO_ROOT, 'core');
const SANDBOX_DOCKER_DIR = join(REPO_ROOT, 'core', 'docker');
const KORTIX_OPENCODE_DIR = join(REPO_ROOT, 'core', 'kortix-master', 'opencode');
...
describe('Deployment — Sandbox Dockerfile Validation', () => {
  ...
  it('core/docker/Dockerfile exists', () => {
  ...
  it('all s6 service run scripts are executable (or Dockerfile handles chmod)', () => {
    ...
    const dockerfile = readFileSync(join(SANDBOX_DOCKER_DIR, 'Dockerfile'), 'utf-8');
  ...
  it('core/package.json version matches release.json', () => {
    const pkgJson = JSON.parse(readFileSync(join(SANDBOX_DIR, 'package.json'), 'utf-8'));
    const release = JSON.parse(readFileSync(join(SANDBOX_DIR, 'release.json'), 'utf-8'));
    expect(pkgJson.version).toBe(release.version);
  });

  it('core/docker/docker-compose.yml documents all required environment variables', () => {
...
describe('Deployment — LocalDockerProvider (mocked)', () => {
  ...
  const providerSource = readFileSync(
    join(REPO_ROOT, 'apps', 'api', 'src', 'platform', 'providers', 'local-docker.ts'),
    'utf-8',
  );
  ...
  it('runs the container in privileged mode', () => {
    expect(providerSource).toContain('Privileged: true');
  });
...
describe('Deployment — Ship Script Validation', () => {
  ...
  it('builds and pushes 3 Docker images with multi-arch', () => {
    ...
    // All 3 images
    expect(shipScript).toContain('core/docker/Dockerfile');
    expect(shipScript).toContain('apps/api/Dockerfile');
    expect(shipScript).toContain('apps/web/Dockerfile');
  });
...
describe.skipIf(!HAS_DOCKER)('Deployment — Docker Compose Validation', () => {
  // Ensure core/docker/.env exists for compose config validation
  ...
  it('root scripts/compose/docker-compose.yml validates successfully', async () => {
    const { exitCode, stderr } = await exec(
      ['docker', 'compose', '-f', 'scripts/compose/docker-compose.yml', 'config', '--quiet'],
      { cwd: REPO_ROOT },
    );
    expect(exitCode).toBe(0);
  });

  it('core/docker/docker-compose.yml validates successfully', async () => {
    const { exitCode, stderr } = await exec(
      ['docker', 'compose', '-f', 'core/docker/docker-compose.yml', 'config', '--quiet'],
      { cwd: REPO_ROOT },
    );
