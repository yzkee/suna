...
export function PresentationToolView({
  ...
  // Extract from toolResult.output if available
  ...
  // Build extracted data from new structure
  const title = args.title || resultData?.title || null;
  const presentation_name = args.presentation_name || args.presentationName || resultData?.presentation_name || resultData?.presentationName || null;
  const message = resultData?.message || null;
