...
// Build the full rehype plugin list, swapping only the sanitize schema
const customRehypePlugins: PluggableList = Object.entries(defaultRehypePlugins).map(
  ([key, plugin]) => {
    if (key === 'sanitize' && Array.isArray(plugin)) {
      const [sanitizePlugin] = plugin;
      return [sanitizePlugin, katexSanitizeSchema];
    }
    return plugin;
  }
) as PluggableList;
