...
function EditTool({ part, defaultOpen, forceOpen, locked }: ToolProps) {
	...
	const before =
		(filediff?.before as string) ?? (input.oldString as string) ?? "";
	const after =
		(filediff?.after as string) ?? (input.newString as string) ?? "";
				...
				<div data-scrollable className="max-h-[400px] overflow-auto">
					...
					{hasResults && (
						<div className="px-3 py-2.5 space-y-2">
							{results.map((result, i) => (
								<div key={i} className="rounded-md bg-muted/10 overflow-hidden">
									{/* Result header */}
					...
					{(scope || parsed.scope) && (
						<div className="px-3 py-1.5 border-t border-border/20 bg-muted/10">
							<span className="text-[10px] text-muted-foreground/50">
								scope:{" "}
								<span className="font-mono">{scope || parsed.scope}</span>
							</span>
						</div>
					)}
				</div>
...
function parseObservationTable(
	output: string,
): { total: number; observations: Observation[] } | null {
	if (!output) return null;
...
function observationTypeInfo(type: string): {
	label: string;
	bg: string;
	text: string;
	dot: string;
							...
							{observations.map((obs, i) => {
								...
								return (
									...
									>
										...
										<div
											className={cn(
												"size-5 rounded-md flex items-center justify-center flex-shrink-0 mt-0.5",
												typeInfo.bg,
											)}
										>
											<span
												className={cn("size-2 rounded-full", typeInfo.dot)}
											/>
										</div>
	...
	for (const section of sections) {
		...
		for (const line of lines) {
			...
			if (
				...
				line.startsWith("**Files read:**") ||
				line.startsWith("**Files modified:**")
			) {
				inNarrative = false;
				continue;
			}
		...
		const filesModifiedLine = lines.find((l) =>
			l.startsWith("**Files modified:**"),
		);
		const filesModified = filesModifiedLine
			? filesModifiedLine
					.replace("**Files modified:**", "")
					.split(",")
					.map((f) => f.trim())
					.filter(Boolean)
			: [];
...
function parseMemGetFileResult(
	output: string,
): { path: string; content: string; total_lines: number } | null {
	if (!output) return null;
	const trimmed = output.trim();
	if (!trimmed.startsWith("{")) return null;
	try {
											...
											{obs.facts.length > 0 && (
												<div className="mt-1.5 space-y-0.5">
													{obs.facts.slice(0, 3).map((fact, fi) => (
														<div
															key={fi}
														 className="text-[10px] text-muted-foreground/50 flex items-start gap-1.5"
														>
															<span className="text-muted-foreground/30 mt-px flex-shrink-0">
																&bull;
															</span>
											...
											{obs.concepts.length > 0 && (
												<div className="flex flex-wrap gap-1 mt-1.5">
													{obs.concepts.slice(0, 5).map((c, ci) => (
														<span
															key={ci}
														 className="text-[9px] px-1.5 py-0.5 rounded-full bg-muted/60 text-muted-foreground/60"
														>
															{c}
														</span>
													))}
...
function MemSaveTool({ part }: ToolProps) {
	...
	const triggerBadge =
		status === "completed" ? (parsed ? parsed.id : "saved") : undefined;
							...
							{entries.map((entry, i) => {
								...
								return (
									...
									>
										...
										<div
										 className={cn(
												"size-5 rounded-md flex items-center justify-center flex-shrink-0 mt-0.5 z-10",
												typeInfo.bg,
											)}
										>
											<span
