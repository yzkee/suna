...
export function SetupOverlay({ onComplete, existingSessionId }: SetupOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background overflow-hidden">
        ...
        {phase === 'login' && (
          ...
          >
            ...
            >
              ...
              <button
                onClick={() => signOut()}
                className="mt-6 flex items-center gap-2 px-3.5 py-2 rounded-full text-[12px] font-medium text-foreground/50 hover:text-foreground/80 bg-foreground/[0.06] hover:bg-foreground/[0.12] backdrop-blur-md border border-foreground/[0.08] transition-all duration-200"
              >
      ...
      {/* Sign out (cloud only — onboarding phase where inline button isn't shown) */}
      {(phase === 'onboarding') && (
        <button
          onClick={() => signOut()}
