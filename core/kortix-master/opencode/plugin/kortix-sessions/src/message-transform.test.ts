...
function makeAssistant(text: string): ChatMessage {
	return { info: { role: "assistant", id: `assistant-${text}` }, parts: [{ type: "text", text }] }
}

function makeSystem(text: string): ChatMessage {
	return { info: { role: "system", id: `system-${text}` }, parts: [{ type: "text", text }] }
}

describe("upsertMemoryContextAtPromptEnd", () => {
	test("appends memory context to the latest user message", () => {
		const messages = [makeSystem("system"), makeUser("older user"), makeAssistant("assistant"), makeUser("latest user")]
	...
	test("replaces existing memory context instead of duplicating it", () => {
		const messages = [makeSystem("system"), { info: { role: "user", id: "user-1" }, parts: [{ type: "text", text: "latest user" }, { type: "text", text: `${MEMORY_CONTEXT_MARKER}\nold` }] }]
		upsertMemoryContextAtPromptEnd(messages, `${MEMORY_CONTEXT_MARKER}\nnew`)
