...
describe("opencode caveman plugin", () => {
	test("switches persistent mode, exposes tools, and compresses files", async () => {
		...
		const parts = { parts: [] as Array<any> }
		await plugin["command.execute.before"]({ command: "caveman", sessionID: call.sessionID, arguments: "ultra" }, parts)
		expect(parts.parts[0]?.text).toContain("Caveman mode set to ultra")
		...
		const review = { parts: [] as Array<any> }
		await plugin["command.execute.before"]({ command: "caveman-review", sessionID: call.sessionID, arguments: "Review this diff" }, review)
		expect(review.parts[0]?.text).toContain("Write terse code review comments")
	...
	test("opencode config wires caveman commands and plugin", async () => {
		const file = await Bun.file(new URL("../../opencode/opencode.jsonc", import.meta.url)).text()
		expect(file).toContain('"caveman"')
		expect(file).toContain('"caveman-commit"')
		expect(file).toContain('"caveman-review"')
		expect(file).toContain('"caveman-compress"')
