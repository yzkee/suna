...
│   ├── MEMORY.md            ← Core memory (4 sections: Identity, User, Project, Scratchpad)
│   ├── SOUL.md              ← Decision principles and values
│   ├── USER.md              ← User profile and preferences
│   ├── mem.db               ← Observation memory (SQLite + FTS5, created by memory plugin)
│   ├── mem/                 ← LSS companion files (obs_*.md, sum_*.md) for semantic search
│   ├── memory/              ← Long-term persistent knowledge (YYYY-MM-DD.md)
│   ├── journal/             ← Session summaries (auto-generated by memory plugin)
...
| `/api/integrations/*` | Kortix API (proxied) | OAuth integration tools (7 endpoints) |
...
| `/kortix/version` | Local | Version info from `/opt/kortix/.version` |
| `/kortix/ports` | Local | Container→host port mappings |
| `/kortix/update/*` | Local | Self-update mechanism |
...
| `vite` | `npm install` | `npm run build` | `npx vite preview --host 0.0.0.0` | 4173 |
| `cra` | `npm install` | `npm run build` | `npx serve -s build` | 3000 |
| `node` | `npm install` | — | `npm start` | 3000 |
| `python` | `pip install -r requirements.txt` | — | `python app.py` | 8080 |
| `static` | — | — | `npx serve -s .` | 3000 |
...
### REST API (Base: http://localhost:3111)

```bash
# List sessions
curl http://localhost:3111/session

# Get session messages
curl http://localhost:3111/session/SESSION_ID/message

# What's running now
curl http://localhost:3111/session/status

# Delete session
curl -X DELETE http://localhost:3111/session/SESSION_ID
```
...
The onboarding command runs automatically on first use. It searches the web for the user, builds a profile in MEMORY.md, and fires `POST /env/ONBOARDING_COMPLETE` to unlock the dashboard.
...
| 99 | `99-memory-init` | Creates `.kortix/` directory structure and seed files (MEMORY.md, SOUL.md, USER.md) |
...
- **Built-in agents disabled**: `build`, `plan`, `explore`, `general` — replaced by `@kortix`
...
- **Provider**: Kortix router (OpenAI-compatible) with two models: `kortix/basic` (Sonnet) and `kortix/power` (Opus)
