...
async function search(q?: string) {
  ...
  if (q) params.set("q", q);
  const res = await fetch(`${masterUrl}/api/integrations/search-apps?${params}`, { headers: authHeaders() });
...
async function list() {
  const res = await fetch(`${masterUrl}/api/integrations/list`, { headers: authHeaders() });
  if (!res.ok) return out({ success: false, error: `${res.status}: ${await res.text()}` });
...
async function actions(app: string, q?: string) {
  ...
  if (q) params.set("q", q);
  const res = await fetch(`${masterUrl}/api/integrations/actions?${params}`, { headers: authHeaders() });
  if (!res.ok) return out({ success: false, error: `${res.status}: ${await res.text()}` });
  ...
  if (!data.actions?.length && q) {
    ...
    const res2 = await fetch(`${masterUrl}/api/integrations/actions?app=${app}`, { headers: authHeaders() });
